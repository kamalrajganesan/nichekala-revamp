...
If `about.html` exists, never create an `about/` folder. Apache's `DirectorySlash` adds a trailing slash → `/about/`, which has no directory index, and `Options -Indexes` is on → **404**. This exact bug caused the recent `projects/` → `projects/` rename: we had both `portfolio.html` (listing) and `projects/` (sub-pages), so `/portfolio` was unroutable.
